        ...
        for obj in objects:
            if obj.type == "ball":
                if (self.pos[0] + self.dx - obj.pos[0]) ** 2 \
                        ...
                        and (self.pos[0] != obj.pos[0] \
                        or self.pos[1] != obj.pos[1]):
                    tmp = (self.dx, self.dy)
            ...
            if obj.type == "wall":
                if intercect_line_circle(self.pos[0] + self.dx, self.pos[1] \
                                         + self.dy, self.radius,
                                         *obj.start, obj.start[0] \
                                         + obj.params[0], obj.start[1]) or \
                   intercect_line_circle(self.pos[0] + self.dx, self.pos[1] \
                                         + self.dy, self.radius,
                                         obj.start[0], obj.start[1] \
                                         + obj.params[1], obj.start[0] \
                                         + obj.params[0], obj.start[1] \
                                         + obj.params[1]):
                    self.dy *= -1
                if intercect_line_circle(self.pos[0] + self.dx, self.pos[1] \
                                         + self.dy, self.radius,
...
def init_walls(surface):
    ...
    wall_list.append(Wall(surface, (WIN_WIDTH - WALL_WIDTH, 0), (WALL_WIDTH, WIN_HEIGHT), RED))
    wall_list.append(Wall(surface, (WALL_WIDTH, WIN_HEIGHT - WALL_WIDTH), (WIN_WIDTH, WALL_WIDTH), RED))
    wall_list.append(Wall(surface, (WIN_WIDTH // 2, WIN_HEIGHT // 2), (WALL_WIDTH, WALL_WIDTH), RED))
    return wall_list
